...
Arthur Baptista dos Santos (565346) · João Pedro (561738) · Nelson Felix (565603)
...
- Escopo controlado: só **leitura/extração de campos** (sem retomar detecção, pois os pesos do
  YOLO não foram persistidos)
...
- Inclinação de câmera e iluminação ruim (reflexo/sub/superexposição) são o que mais derruba a
...
- Hipótese inicial: o erro do OCR clássico era "1 caractere errado no separador"; o regex foi
  tornado mais tolerante (`/` lido como `|`/traço, `kW`/`Hz`/`IP` com espaço entre letras)
