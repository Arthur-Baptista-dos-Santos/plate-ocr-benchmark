...
Ou seja: a **detecção** (localizar a placa na cena) funcionou bem; a **leitura de campos**
...
| Chave de API | — | — | lida de variável de ambiente `OPENAI_API_KEY` (nunca hardcoded no repositório) |
...
explicitly na análise de erros (seção 6).
...
- **Character-level Accuracy (1 − CER)**: baseada em distância de edição de Levenshtein sobre o
  ...
  não é um acerto exato — relevante para diagnosticar *por que* uma abordagem erra, não só que
  errou.
...
resolução), a abordagem multimodal também degrada bastante — de 100% para 34% — mas ainda assim
permanece muito acima do Tesseract (1,0%) e do EasyOCR (0,0%), que praticamente zeram.
...
`grau_ip` (regex `IP\d{2}`) são os piores — exatamente os campos cujo regex depende de um
separador (`/`) ou prefixo (`IP`) ser lido character-perfect; qualquer falha de segmentação de
caractere nesses pontos já invalida o campo inteiro. `num_serie` continua sendo o campo mais
difícil mesmo para o modelo multimodal (66,7%, o mais baixo entre os campos avaliados) — é o
único campo alfanumérico longo e sem vocabulário fechado (não dá para "adivinhar" contra uma
lista curta como se faz com fabricante/modelo).
...
- **EasyOCR ficou sistematicamente abaixo do Tesseract** em todos os níveis de dificuldade e em
  ...
  teste maior e mais adverso — achado que só apareceu por termos comparado abordagens
  lado a lado desta vez.
- **Modelo multimodal (leitura direta):** único ponto realmente fraco é o nível "difícil"
  ...
  valor errado), concentrada em imagens com combinação de baixa resolução + reflexo/subexposição
  forte, onde o texto fica genuinamente ilegível mesmo para leitura humana da imagem re-renderizada
  em baixa qualidade. Ou seja, quando erra, tende a **admitir que não leu** em vez de alucinar um
  valor — comportamento desejável para uma aplicação real (evita popular o banco de ativos com
  dado errado com alta confiança), mas precisa ser validado formalmente com mais dados antes de
  virar uma afirmação geral sobre o modelo.
